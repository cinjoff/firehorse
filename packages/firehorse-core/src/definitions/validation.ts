...
export function validateDefinitionSet(
  ...
  const workflowNativeNames = new Map<string, FirehorseDefinition>();

  for (const definition of definitions) {
    if (definition.kind === "workflow") {
      for (const nativeName of [
        `horse-${definition.frontmatter.id}`,
        ...(definition.frontmatter.nativeAliases ?? []),
      ]) {
        const existing = workflowNativeNames.get(nativeName);
        if (existing) {
          diagnostics.push({
            code: "set.workflow_native_name_collision",
            message: `Workflow native name '${nativeName}' is generated by both '${existing.path}' and '${definition.path}'. Native workflow names must be unique across canonical IDs and nativeAliases.`,
            path: definition.path,
            field: "nativeAliases",
          });
        } else {
          workflowNativeNames.set(nativeName, definition);
        }
      }
    }
    ...
    if (
      ...
      diagnostics.push({
        ...
        message: `Agent role name '${definition.frontmatter.name}' must match id '${definition.frontmatter.id}' in canonical definitions. Generic agent-role projections use provider-native names directly.`,
        path: definition.path,
        field: "name",
      });
    }
