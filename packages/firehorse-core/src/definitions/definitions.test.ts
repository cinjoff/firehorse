...
describe("Firehorse definitions", () => {
  it("parses and validates a canonical workflow and skill fixture set", () => {
    ...
    expect(definitions.map((definition) => definition.kind).sort()).toEqual(["skill", "workflow"]);
  });
  ...
  it("validates duplicate ids and structured references across a definition set", () => {
    const workflow = parseWorkflow("diagnose-fix", "supportingSkills:\n  - id: feedback-loop\n");
  ...
  it("projects Claude mirrors and manifest entries deterministically", () => {
    ...
    expect(
      projectDefinitions([...definitions].reverse(), { repoRoot }).map((file) => file.path),
    ).toEqual(generated.map((file) => file.path));
  });

  it("projects a maintainer definition into .claude/ and keeps it out of the manifest", () => {
    const definitions = [
      parseWorkflow("diagnose-fix"),
      parseWorkflow("release-it", "audience: maintainer\n"),
      parseSkill("feedback-loop"),
      parseSkill("maintainer-notes", "audience: maintainer\n"),
    ];
    assertValidDefinitionSet(definitions);
    ...
    expect(generated.map((file) => file.path)).toEqual([
      ".claude/commands/release-it.md",
      ".claude/skills/maintainer-notes/SKILL.md",
      "packages/firehorse-claude/commands/firehorse/diagnose-fix.md",
      "packages/firehorse-claude/skills/firehorse/feedback-loop/SKILL.md",
    ]);
    expect(generated.map((file) => file.audience)).toEqual([
      "maintainer",
      "maintainer",
      "user",
      "user",
    ...
    // A maintainer mirror is a full mirror; only its destination differs.
    expect(generated[0]?.content).toContain("Generated by Firehorse. DO NOT EDIT.");
    expect(generated[0]?.content).toContain("## Procedure");
  ...
  it("defaults an undeclared audience to user", () => {
    const [generated] = projectDefinitions([parseWorkflow("diagnose-fix")], { repoRoot });

    expect(generated?.audience).toBe("user");
    expect(generated?.path).toBe("packages/firehorse-claude/commands/firehorse/diagnose-fix.md");
  });

  it("rejects an audience outside the declared vocabulary", () => {
    expect(() => parseWorkflow("diagnose-fix", "audience: everyone\n")).toThrow(
      DefinitionValidationError,
    );
  });

  it("keeps Projection Notes out of the mirror the agent loads", () => {
    const definitions = [parseWorkflow("diagnose-fix"), parseSkill("feedback-loop")];
    const generated = projectDefinitions(definitions, { repoRoot });

    for (const file of generated) {
      expect(file.content).not.toContain("## Projection Notes");
      expect(file.content).toContain("Generated by Firehorse. DO NOT EDIT.");
    }

    // Every other required section survives.
    expect(generated[0]?.content).toContain("## Safety Gates");
    expect(generated[1]?.content).toContain("## Boundaries");
  });

  it("ignores headings inside fenced blocks when stripping sections", () => {
    const body = [
      "## Procedure",
      "",
      "```markdown",
    ...
    const stripped = stripAuthoringOnlySections(body);
    expect(stripped).toContain("```markdown\n## Projection Notes\n```");
    expect(stripped).not.toContain("Authoring detail.");
  });
  ...
  it("writes generated manifest entries in deterministic sorted order", () => {
    ...
    expect(
      generatedManifestEntries(
        [...definitions]
          .reverse()
          .flatMap((definition) => projectDefinitions([definition], { repoRoot })),
      ),
