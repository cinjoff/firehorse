...
- Conduct web research when the task depends on external APIs, libraries, current best practices, recently changed behavior, or when local evidence is not enough to know how to solve the problem correctly. Use web research if it is available; otherwise use whatever equivalent research capability is available.
...
- stop/escalation rules: when to ask the supervisor, when enough evidence is enough, and when to stop
